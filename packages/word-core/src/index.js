import { AlignmentType, BorderStyle, Footer, ImageRun, PageNumber, Paragraph, Table, TableCell, TableLayoutType, TableRow, TextRun, WidthType } from "docx";
...
function chunkRowsByHeight(rows, firstPageBudget, followPageBudget) {
    const pages = [];
    let currentPage = [];
...
function createRow(item, assets) {
    const photos = assets
...
async function readZipDataUrl(zip, path) {
...
export async function loadWordTemplateAssets() {
...
function getDirectChildElements(node) {
    return Array.from(node.childNodes).filter((child) => child.nodeType === Node.ELEMENT_NODE);
}
function cloneWithText(doc, paragraph, text, color) {
    const clone = paragraph.cloneNode(true);
    const pPr = clone.getElementsByTagNameNS(WORD_NS, "pPr")[0]?.cloneNode(true) ?? null;
    const runPrSource = clone.getElementsByTagNameNS(WORD_NS, "rPr")[0]?.cloneNode(true);
    ...
    if (runPrSource) {
        const runPr = runPrSource.cloneNode(true);
        if (color) {
...
function setParagraphsInCell(doc, cell, texts) {
...
function appendAddress(table, addressLines) {
...
function appendDateValue(table, value) {
...
function setFooterClerkName(node, value) {
...
function ensureImageRelationship(zip, relsDoc, photo, counter) {
...
function buildTemplateObjectTable(doc, templateTable, row, imageRelationshipId) {
...
function createDocxPhotoRuns(photos) {
    if (!photos.length) {
        return null;
    }
    const columns = 4;
...
function createDocxSections(model) {
...
function drawWrappedText(page, font, text, x, y, maxWidth, lineHeight, size, color = rgb(0.15, 0.21, 0.18)) {
...
async function drawPdfPhoto(page, pdfDocument, photo, x, y, size) {
    const parsed = dataUrlToBytes(photo.src);
    if (!parsed) {
        return;
    }
...
async function drawPdfRow(page, pdfDocument, font, row, x, y) {
...
export async function generateWordDocx(caseFile, masterData) {
    ...
    model.pages.forEach((page, pageIndex) => {
        ...
        if (page.showAddress) {
            const addressClone = addressTable.cloneNode(true);
        ...
        const dateClone = dateTable.cloneNode(true);
        ...
        footerNodes.forEach((footerNode, footerIndex) => {
            const footerClone = footerNode.cloneNode(true);
            if (footerIndex === 1) {
...
export async function generateWordPdf(caseFile, masterData) {
    const model = createWordPreviewModel(caseFile, masterData);
