...
import { deriveAddressLines, deriveBeneficiary, formatAmountForDisplay } from "@elb/domain/index";
import templatePdfUrl from "../../../vorlagen/template.pdf?url";
import templateObjectsPdfUrl from "../../../vorlagen/template_objekte.pdf?url";
function getAuctionLabel(auction) {
    if (!auction) {
        return "";
    }
...
function getPriceLabel(auction, objectItem) {
...
function getDepartmentCode(departments, departmentId) {
    return getDepartment(departments, departmentId)?.code ?? "";
}
function collectMissingRequiredFields(caseFile, masterData) {
    const missing = [];
    for (const field of masterData.globalPdfRequiredFields) {
        if (field === "meta.receiptNumber" && !caseFile.meta.receiptNumber.trim())
            missing.push("ELB-Nummer");
        if (field === "meta.clerkId" && !caseFile.meta.clerkId.trim())
            missing.push("Sachbearbeiter");
        if (field === "consignor.lastName" && !caseFile.consignor.lastName.trim())
            missing.push("Nachname Einlieferer");
        if (field === "consignor.street" && !caseFile.consignor.street.trim())
            missing.push("Straße Einlieferer");
        if (field === "consignor.zip" && !caseFile.consignor.zip.trim())
            missing.push("PLZ Einlieferer");
        if (field === "consignor.city" && !caseFile.consignor.city.trim())
            missing.push("Stadt Einlieferer");
    }
    caseFile.objects.forEach((item, index) => {
        if (!item.departmentId.trim())
            missing.push(`Objekt ${index + 1}: Abteilung`);
        if (!item.shortDescription.trim())
            missing.push(`Objekt ${index + 1}: Kurzbeschrieb`);
        if (!item.estimate.low.trim())
            missing.push(`Objekt ${index + 1}: Schätzung von`);
        if (!item.estimate.high.trim())
            missing.push(`Objekt ${index + 1}: Schätzung bis`);
    });
...
function setMultilineTextFieldSafe(form, fieldName, value) {
    try {
...
function buildCostFieldValue(cost) {
...
function getFieldRects(form, fieldName) {
    try {
...
function ensurePage(pdf, currentPage, cursor, requiredHeight, margin) {
...
async function drawSignatureIntoFields(pdf, pageIndex, form, fieldNames, dataUrl) {
    if (!dataUrl.trim()) {
        return;
    }
...
function toHotspotRect(union, pageWidth, pageHeight) {
    return {
...
function getUnionForFields(form, fieldNames) {
...
function buildObjectFieldLayout(row, font, widths, fontSize) {
...
function getRequiredRect(form, fieldName) {
...
function getObjectFieldGeometry(form, suffix) {
...
export async function buildObjectPageChunks(rows) {
    ...
    const chunks = [];
    let current = { intNumber: "", auctionLabel: "", departmentCode: "", description: "", estimate: "", items: [], usedLines: 0, capacityLines: mainCap };
    let usedLines = 0;
...
export async function getPdfHotspotMap(pageKind) {
    ...
    function buildObjectHotspot(fields) {
...
function fillSharedFields(form, caseFile, masterData, pageNumber, totalPages) {
...
function fillObjectFields(form, row, suffix) {
...
export function createPdfPreviewModel(caseFile, masterData) {
    const clerk = masterData.clerks.find((item) => item.id === caseFile.meta.clerkId);
    return {
...
export async function generateElbPdf(caseFile, masterData) {
...
export async function generateSupplementPdf(caseFile, masterData) {
    ...
    const cursor = { y: pageHeight - margin };
    const clerk = masterData.clerks.find((item) => item.id === caseFile.meta.clerkId);
    ...
    for (const [index, objectItem] of caseFile.objects.entries()) {
        ...
        page.drawText([department ? `${department.code} ${department.name}` : "", auction ? getAuctionLabel(auction) : ""].filter(Boolean).join(" · "), {
            x: margin + 90,
            y: cursor.y - 18,
            size: 9.5,
            font,
        ...
        const blockCursor = { y: cursor.y - 38 };
        drawWrappedBlock({
            page,
            font,
            boldFont,
    ...
    if (closingLines.length > 2) {
        ...
        const closingCursor = { y: pageHeight - margin };
